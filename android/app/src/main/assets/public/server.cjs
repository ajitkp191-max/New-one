...
function callWithTimeout(promise, timeoutMs) {
  let timeoutHandle;
...
async function callGeminiResilient(options) {
  ...
  for (let i = 0; i < candidateModels.length; i++) {
    ...
    try {
      ...
      const isHighDemandOrThrottled = errCode === 503 || errCode === "UNAVAILABLE" || errCode === 429 || errCode === "RESOURCE_EXHAUSTED" || errMsg.includes("503") || errMsg.includes("high demand") || errMsg.includes("UNAVAILABLE") || errMsg.includes("429") || errMsg.includes("Timeout");
...
function sanitizeBase64(dataUrlOrRaw) {
...
6. Format responses with clear Markdown headings and bullet points for effortless readability.`;
  ...
  if (imageBase64) {
    ...
    const conversationParts = history.slice(-6).map((msg) => `${msg.sender === "user" ? "User" : "Assistant"}: ${msg.text}`).join("\n\n");
...
User: ${prompt}`;
  ...
  const aiText = await callGeminiResilient({
    ...
    systemInstruction,
    temperature: 0.7,
    timeoutMs: imageBase64 ? 14e3 : 8e3
  });
  if (aiText) {
  ...
  let responseText = "";
  if (imageBase64) {
    responseText = `### \u{1F4F8} Visual Symptom Screening Analysis
...
* **When to Seek Immediate Care**: If the area swells, develops purulent drainage, or your pet becomes lethargic or anorexic, book a clinic appointment immediately.`;
    ...
    responseText = `### \u{1FA7A} Dental Wellness Plan \u2014 Dr. Helen Miller (Dentistry Specialty)
*Patient: ${petContext?.name || "Your Pet"} (${petContext?.species || "Companion"})*
...
* **Home Regimen**: Introduce enzymatic pet toothpaste 3x weekly (never human toothpaste). Consider VOHC-approved dental chews.`;
    ...
    responseText = `### \u{1F357} Advanced Clinical Nutrition & Dietary Protocol
...
* **Emergency Transport**: Proceed immediately to the nearest veterinary hospital or contact the clinic emergency hotline directly at **+1 (555) 019-2834**.`;
    ...
    responseText = `### \u{1F43E} Advanced Companion Wellness Advice
*Patient: ${petContext?.name || "Your Pet"} (${petContext?.species || "Companion"})*
...
4. Always conclude with: "Clinical decision support only. Licensed veterinarian retains final clinical authority."`;
  ...
  const aiText = await callGeminiResilient({
    ...
    systemInstruction,
    temperature: 0.2,
    timeoutMs: imageBase64 ? 15e3 : 9e3
  });
  if (aiText) {
    return res.json({ text: aiText, source: "gemini" });
  }
...
*Clinical decision support only. Licensed veterinarian retains final clinical authority.*`;
    ...
    responseText = `### \u{1F48A} Clinical Pharmacology & Drug Interaction Analysis
...
*Clinical decision support only. Licensed veterinarian retains final clinical authority.*`;
    ...
    responseText = `### \u{1FA7A} Comprehensive Clinical Decision Support
...
app.post("/api/ai/doctor", async (req, res) => {
  ...
  res.json({
    text: `### \u{1FA7A} Rapid Veterinary Clinical Decision Support
...
Always conclude with a professional clinical disclaimer.`;
  ...
  const fallbackResult = `### \u{1F52C} Multimodal Veterinary Visual Analysis
...
app.post("/api/ai/drug-checker", async (req, res) => {
  ...
  const drugListStr = drugs.map((d) => typeof d === "string" ? d : `${d.name} (${d.doseRate || "standard dose"}, ${d.route || "PO"})`).join(", ");
...
5. Required Clinical Monitoring (e.g. renal chemistry, hydration, ECG)`;
  ...
  const fallbackResult = `### \u{1F48A} Advanced Drug Interaction & Safety Verification
*Patient: ${patient?.name || "Patient"} (${patient?.species || "Canine"}, ${patient?.weight || 10} kg)*
*Regimen: ${drugListStr}*

#### \u{1F6A6} Overall Safety Status: **${safetyStatus}**
...
2. Output must be strictly valid JSON in the format:
{
  ...
  const aiText = await callGeminiResilient({
    ...
    systemInstruction,
    temperature: 0.1,
    timeoutMs: imageBase64 ? 18e3 : 1e4
  });
  if (aiText) {
    try {
  ...
  const fallbackData = {
    ...
    prescriptionDate: (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
...
Recheck: ${recheckDate || "7-10 days"}`;
  ...
  const fallback = `### \u{1F4CB} Veterinary Hospital Discharge Instructions
*Patient: ${patient?.name || "Companion"} | Date: ${(/* @__PURE__ */ new Date()).toLocaleDateString()}*

**Condition / Diagnosis:** ${diagnosis || "Routine Treatment & Observation"}

---

#### \u{1F48A} Medication Administration Schedule:
${medications.map((m, i) => `* **${m.drug || m.name}**: Give as directed (${m.route || "PO"} - ${m.freq || "daily"}). Administer with a small meal.`).join("\n") || "* No take-home medications required."}

---

#### \u{1F3E1} Home Care & Activity Guidelines:
...
#### \u{1F6A8} Warning Signs to Watch For:
...
- Body Temperature: ${vitals?.temp || 38.5} \xB0C (Normal: 38.0 - 39.2 \xB0C)
...
Vitals: Temp ${vitals?.temp}\xB0C, Pulse ${vitals?.pulse} bpm, Resp ${vitals?.resp} brpm, CRT ${vitals?.crt}, Mucous Membranes: ${vitals?.mucousMembranes}, Hydration: ${vitals?.hydration}
...
4. Itemized Prescription Items (Drug name, dose rate, route, frequency, duration, special instructions).`;
  ...
  if (aiText) {
    return res.json({
      success: true,
      result: aiText,
      treatments: step === "confirmatory_with_labs" ? treatments : void 0,
      confirmatoryDx: step === "confirmatory_with_labs" ? confirmatoryDx : void 0,
      source: "gemini"
    });
  }
  if (step === "initial_differentials") {
    let differentials = [];
    let labRecommendations = [];
    let reason = "";
    if (cat.includes("digestive") || cat.includes("gastro") || symptoms.some((s) => /vomit|diarrhea|anorexia|appetite|bloat/i.test(s))) {
      differentials = [
    ...
    } else if (cat.includes("cardiac") || cat.includes("heart") || symptoms.some((s) => /cough|murmur|faint|syncope|exercise/i.test(s))) {
      differentials = [
    ...
    } else if (cat.includes("dermal") || cat.includes("skin") || symptoms.some((s) => /pruritus|scratch|hair|alopecia|erythema|hotspot/i.test(s))) {
      differentials = [
    ...
    } else if (cat.includes("respiratory") || cat.includes("lung") || symptoms.some((s) => /breath|dyspnea|sneeze|wheez|stridor/i.test(s))) {
      differentials = [
    ...
    const outputText2 = `### \u{1FA7A} Advanced AI Diagnostic Analysis: Initial Differentials
...
#### \u{1F4CB} Suspected Causes & Differential Diagnoses:
${differentials.map((d, i) => `${i + 1}. **${d.name}** \u2014 *Estimated Probability: ${d.probability}*
   \u2022 **Clinical Justification:** ${d.reason}`).join("\n\n")}

---

#### \u{1F9EA} Recommended Laboratory & Smear Diagnostics:
${labRecommendations.map((lr) => `* \u{1F52C} **${lr}**`).join("\n")}

> \u{1F4A1} **Doctor Action Required:** Please perform or upload the laboratory reports (e.g. CBC, Serum Chemistry, Blood Smear Findings) below to generate the Confirmatory Diagnosis and precise Line of Treatment with calculated Dose Rates.`;
    return res.json({ success: true, result: outputText2, source: "fallback_engine" });
  }
  ...
  const outputText = `### \u{1F3AF} Confirmatory Diagnosis & Clinical Line of Treatment

**Definitive Diagnosis:**
# \u{1FA7A} ${confirmatoryDx}

**Clinical Reasoning:**
${reasoning} Laboratory parameters and microscopic smear findings were evaluated alongside physical vitals (Temp: ${vitals?.temp}\xB0C, HR: ${vitals?.pulse} bpm). Key differential exclusions have been confirmed based on normal/abnormal markers.

---

### \u{1F48A} Proper Line of Treatment & Precise Dose Rates:
...
### \u{1F4DD} Auto-Generated Prescription Ready:
...
app.get("/api/notifications/stream", (req, res) => {
  ...
  res.write(`data: ${JSON.stringify({ notificationId: "system-init", title: "Connected", message: "VetPulse Pro Real-Time Alert Engine initialized.", type: "general", date: (/* @__PURE__ */ new Date()).toISOString() })}
